...
class GalleryApiRepository {
  ...
  /// Rename gallery
  Future<Map<String, dynamic>> renameGallery(
    String galleryId,
    String newTitle,
  ) async {
    final options = await _getAuthOptions();
  ...
  Future<void> deleteGallery(String galleryId) async {
    final options = await _getAuthOptions();
    await _apiClient.delete('/api/galleries/$galleryId', options: options);
  }
  ...
  Future<void> deletePhoto(String photoId) async {
    final options = await _getAuthOptions();
    await _apiClient.delete('/api/photos/$photoId', options: options);
  }
  ...
  // ============================================================================
  // ADMIN ENDPOINTS (Board Members Only)
  // ============================================================================

  /// Get admin statistics (Board members only)
  Future<Map<String, dynamic>> getAdminStats() async {
    final options = await _getAuthOptions();
    final response = await _apiClient.get('/api/admin/stats', options: options);
    return response.data;
  }

  /// Get admin activity feed (Board members only)
  Future<Map<String, dynamic>> getAdminActivity({int limit = 20}) async {
    final options = await _getAuthOptions();
    final response = await _apiClient.get(
      '/api/admin/activity',
  ...
  /// Get admin audit logs (Board members only)
  Future<Map<String, dynamic>> getAdminLogs({int limit = 100}) async {
    final options = await _getAuthOptions();
    final response = await _apiClient.get(
      '/api/admin/logs',
      queryParameters: {'limit': limit},
  ...
  /// Get orphaned photos (Board members only)
  Future<Map<String, dynamic>> getOrphanedPhotos() async {
    final options = await _getAuthOptions();
    final response = await _apiClient.get(
      '/api/admin/maintenance/orphaned-photos',
      options: options,
    );
    return response.data;
  }

  /// Cleanup old galleries (Board members only)
  Future<Map<String, dynamic>> cleanupGalleries() async {
    final options = await _getAuthOptions();
    final response = await _apiClient.post(
      '/api/admin/maintenance/cleanup-galleries',
      options: options,
    );
    return response.data;
  }

  /// Optimize system (Board members only)
